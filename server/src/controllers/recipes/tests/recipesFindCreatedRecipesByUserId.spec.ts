...
const { findCreatedRecipesByUserId } = createCaller({ db });

it('Should return an empty list if there are no recipes created by user', async () => {
  expect(await findCreatedRecipesByUserId({ userId: userOne.id })).toHaveLength(
    0
  );
});
...
it('Should return a list of recipes', async () => {
  ...
  const recipes = await findCreatedRecipesByUserId({ userId: userOne.id });
